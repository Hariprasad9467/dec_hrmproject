...
class _AdminDashboardState extends State<AdminDashboard> {
  ...
  Future<void> fetchEmployeeName() async {
    ...
    try {
      // ✅ FIX: Use the production URL and correct endpoint to fetch the employee name.
      final response = await http.get(
        Uri.parse("https://dec-hrmproject.onrender.com/api/employees/$employeeId"),
      );

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        setState(() {
          // Safely access the employeeName, which might be nested or have a different key.
  ...
  Future<void> fetchPendingCount(String userRole, String employeeId) async {
    try {
      ...
      //   ),
      // );
      final response = http.Response(jsonEncode({'pendingCount': 3}), 200);
      ...
      child: Wrap(
        ...
        children: [
          _quickActionButton('Apply Leave', () {
            // Navigate to ApplyLeave screen, which will get user info from Provider
            Navigator.push(
              context,
              ...
              children: [
                _quickActionButton('Leave Approval', () {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) =>
                          const LeaveApprovalPage(userRole: "tl"),
                    ),
                  ).then((_) {
