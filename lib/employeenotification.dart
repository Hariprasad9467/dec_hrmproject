...
class _EmployeeNotificationsPageState extends State<EmployeeNotificationsPage> {
  ...
  Future<void> fetchNotifs() async {
    ...
    */
    try {
      ...
      // This will catch any error from the fetches and prevent a crash.
      debugPrint("--- ❌ CRITICAL ERROR in fetchNotifs: $e ---");
      debugPrint("--- StackTrace: $stackTrace ---");
      if (mounted) {
        setState(
          () => error = "Failed to load some notifications. Please try again.",
        );
      }
    } finally {
  ...
  Future<void> fetchPerformanceNotifications() async {
    final uri = Uri.parse(
      "$apiBaseUrl/api/notifications/employee/${widget.empId}?month=$selectedMonth&category=performance",
    );
    final resp = await http.get(uri);
  ...
  Future<void> fetchHolidayNotifications() async {
    final uri = Uri.parse(
      "$apiBaseUrl/api/notifications/holiday?month=$selectedMonth", // Corrected URL
    );
    final resp = await http.get(uri);
