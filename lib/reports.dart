...
class _ReportsAnalyticsPageState extends State<ReportsAnalyticsPage> {
  ...
  Future<void> fetchPerformanceReviews([String? empId]) async {
    ...
    try {
      ...
      final uri = Uri.parse('$apiBaseUrl/api/reports/employee/$employeeId');
  ...
  Future<Map<String, dynamic>?> fetchReviewDetails(String id) async {
    try {
      final uri = Uri.parse('$apiBaseUrl/api/reports/$id');
      final response = await http.get(
        uri,
        headers: {'Accept': 'application/json'},
      );
  ...
  Future<void> _sendDecision({
    ...
    await http.post(
      Uri.parse('$apiBaseUrl/api/review-decision'),
  ...
  Future<void> fetchWorkProgress() async {
    ...
    var url = Uri.parse('$apiBaseUrl/api/todo_planner/todo/progress/$employeeId');
    try {
  ...
  Future<void> fetchLeaveStatus() async {
    ...
    var url = Uri.parse('$apiBaseUrl/api/apply/leave-balance/$employeeId'); // updated
    try {
      var response = await http.get(url);
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
